...
pub struct Track {
    ...
    /// Byte range of the authoring `track "..." (...)` header in the source
    /// `.mog` file (active source for user-authored clips, the imported file
    /// for clips lifted from a module). `None` for tracks generated by
    /// procedural templates (`spin`/`open_close`/...) which have no track
    /// header to write back to. Studio's gizmo uses this to splice
    /// `axis=`/`from=`/`to=` updates back onto the originating track.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub source_span: Option<Span>,
}

impl Track {
    /// True when every keyframe in the track encodes the same value (within
    /// `eps`). Constant tracks are gizmo-editable: the writeback collapses
    /// to a single `axis=`/`from=`/`to=` triple. Non-constant tracks would
    /// need keyframe-aware editing and currently fall through to the
    /// node-rest-pose writeback path.
    pub fn is_constant_value(&self, eps: f32) -> bool {
