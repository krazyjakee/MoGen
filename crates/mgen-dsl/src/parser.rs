...
fn build_node(pair: Pair<Rule>) -> Result<Node> {
    ...
    Ok(Node { kind, name, attrs, children, span, kind_span })
}
...
fn build_list(val_pair: Pair<Rule>) -> Result<Value> {
        ...
        match inner.as_rule() {
            ...
            Rule::list => {
                ...
                match consts.len() {
                    ...
                    n => {
                        return Err(anyhow!(
                            "nested list must have 2 or 3 components, got {n}"
                        ))
                    }
                }
            }
    ...
    if (has_vec3 || has_pair) && has_expr {
        return Err(anyhow!(
            "list items must be all scalars or all nested vec3/pair — not mixed"
        ));
    }
    if has_vec3 && has_pair {
        return Err(anyhow!(
            "list items must all be the same arity — mixing 2-element and 3-element is not allowed"
        ));
    }
