...
//!
//! Path resolution:
//!   - `src="stdlib:humanoid/head_male_casual.glb"` — pulled from the embedded
//!     stdlib map (`HUMANOID_ASSETS`, generated by `build.rs`).
...
/// Prefix that selects an embedded stdlib asset rather than a filesystem path.
...
pub fn read_glb_bytes(src: &str, source_dir: Option<&Path>) -> Result<Vec<u8>> {
    if let Some(key) = src.strip_prefix(STDLIB_PREFIX) {
        return humanoid::stdlib_asset(key)
            .ok_or_else(|| anyhow!("unknown stdlib asset: {key}"))
            .map(|b| b.to_vec());
    }
...
mod tests {
    ...
    #[test]
    fn loads_embedded_humanoid_torso() {
        let bytes = humanoid::stdlib_asset("humanoid/torso_male_casual.glb")
            .expect("torso_male_casual.glb is registered");
        let mesh = mesh_from_glb_bytes(bytes).expect("imports cleanly");
        // Sanity: positions/normals/indices line up.
        assert!(!mesh.positions.is_empty(), "torso mesh has positions");
        assert_eq!(mesh.normals.len(), mesh.positions.len());
        assert!(mesh.indices.len() >= 3, "at least one triangle");
        assert_eq!(mesh.indices.len() % 3, 0, "indices come in tri triples");
        // Bounds match what we measured at bake time (within a small tol).
        ...
        // Bake measured the head_male_casual at y in [4.118, 4.676]; torso
        // sits below that. We just want a non-degenerate AABB.
        for i in 0..3 {
            assert!(max[i] > min[i], "axis {i} has positive extent");
        }
    }

    #[test]
    fn read_glb_bytes_resolves_stdlib_prefix() {
        let bytes = read_glb_bytes("stdlib:humanoid/foot_l_male_casual.glb", None)
            .expect("stdlib lookup");
        assert!(bytes.starts_with(b"glTF"), "is a glb");
    }

    #[test]
    fn read_glb_bytes_unknown_stdlib_errors() {
        let err = read_glb_bytes("stdlib:humanoid/does_not_exist.glb", None).unwrap_err();
        assert!(format!("{err}").contains("unknown stdlib asset"));
    }

    #[test]
    fn embedded_table_has_158_humanoid_assets() {
        // Lock in the dedup result so an accidental rebake adding/removing
        // files trips this test instead of silently changing behaviour.
        let n = humanoid::stdlib_keys().count();
        assert_eq!(n, 158, "expected 158 deduped humanoid GLBs, got {n}");
    }
}
