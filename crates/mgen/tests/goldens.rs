...
//!
//! Each canonical example under `examples/<name>.mg` is compiled through the
...
//! the generator's output has drifted — rerun `mgen build` on the example and
//! inspect the diff before committing the new snapshot.
//!
//! Update mode: set `MGEN_GOLDENS_UPDATE=1` and the test overwrites the
...
fn workspace_root() -> PathBuf {
    // CARGO_MANIFEST_DIR points at crates/mgen; climb two to the workspace.
...
fn compile_example(name: &str) -> Vec<u8> {
    ...
    let src_path = root.join("examples").join(format!("{name}.mg"));
    let src = fs::read_to_string(&src_path)
        .unwrap_or_else(|e| panic!("reading {}: {e}", src_path.display()));

    let ast = mgen_dsl::parse(&src).expect("parse failed");
    let diags = mgen_validate::validate_ast(&ast);
    assert!(
        !mgen_core::has_errors(&diags),
        "{name}: validation errors: {}",
        mgen_validate::render_json("<test>", &diags)
    );
    let scene = mgen_dsl::lower(&ast).expect("lower failed");

    let tmp = env::temp_dir().join(format!(
        "mgen-goldens-{}-{name}.glb",
        std::process::id()
    ));
    mgen_export::write_glb(&scene, &tmp).expect("write_glb failed");
...
fn check_golden(name: &str) {
    ...
    if env::var_os("MGEN_GOLDENS_UPDATE").is_some() {
    ...
    let expected = fs::read(&golden)
        .unwrap_or_else(|e| panic!("no committed golden at {}: {e}. Run with MGEN_GOLDENS_UPDATE=1 to create.", golden.display()));

    if produced != expected {
        let diff_path = env::temp_dir().join(format!("mgen-goldens-{name}.actual.glb"));
        let _ = fs::write(&diff_path, &produced);
        panic!(
            "{name}: GLB bytes differ from {} (got {} bytes, expected {}). Actual output written to {}. Re-run with MGEN_GOLDENS_UPDATE=1 once the change is intentional.",
            golden.display(),
            produced.len(),
            expected.len(),
            diff_path.display()
...
fn try_gltf_validator(path: &Path) {
    let binary = env::var("MGEN_GLTF_VALIDATOR").unwrap_or_else(|_| "gltf_validator".into());
