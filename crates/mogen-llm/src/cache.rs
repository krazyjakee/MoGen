...
//!
//! Re-reading this map across invocations lets `mogen generate` / `modify` /
...
/// Default on-disk location: `$MOGEN_CACHE_DIR/gemini.json` if set, else
/// `$HOME/.cache/mogen/gemini.json`. Returns `None` when neither is available
...
pub fn default_cache_path() -> Option<PathBuf> {
    ...
    std::env::var("HOME")
        ...
        .map(|h| PathBuf::from(h).join(".cache").join("mogen").join("gemini.json"))
}
...
mod tests {
    ...
    fn load_missing_file_returns_default() {
        let f = CacheFile::load(Path::new("/nonexistent/mogen/cache.json"));
    ...
    #[test]
    fn default_cache_path_prefers_mogen_cache_dir() {
    ...
    fn tempdir_like() -> PathBuf {
        let ns = SystemTime::now()
            ...
            .map(|d| d.as_nanos())
            .unwrap_or(0);
        let p = std::env::temp_dir().join(format!("mogen-llm-cache-test-{ns}"));
