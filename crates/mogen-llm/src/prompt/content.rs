...
| `sweep` | `profile=[[x,y], …]` (closed CCW), `path=[[x,y,z], …]` (Catmull–Rom centreline) | `samples` (8), `twist` deg (uniform total roll), `roll=[deg, …]` (one constant or one per path control point; triples work), `scale_along=[s, …]` (one constant or one per path point, finite nonnegative), `caps=0\\|1`, `frame_up=[0,1,0]` (local profile height; width = height × tangent), `closed=1` (requires frame_up, seamless loop); omit frame_up for legacy orientation. |
