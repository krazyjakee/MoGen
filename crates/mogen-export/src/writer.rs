...
fn emit_node(
    ...
    // every derived one above, `shader` included. That is deliberate: it lets a
    // converter or an engine-specific payload override `role`/`tags` without us
    // having to anticipate every key someone might want to set.
