...
pub struct ExportOptions {
    ...
    /// Generate three simplified LOD stages per non-skinned mesh node, attached
    /// via the `MSFT_lod` glTF extension (Godot 4 imports it natively).
    /// Does not bake an imposter spritesheet — use `bundle_lods_and_imposter`
    /// when you also want the billboard quad. No display server required.
    pub bundle_lods: bool,
    ...
    /// LODs attach via the `MSFT_lod` extension; the imposter is an orphan
    /// top-level node tagged `"imposter"` in `extras.tags`, carrying a 1×N
    /// yaw-grid spritesheet baked from the headless renderer. Requires a
    /// real display server (X11 or Wayland) at runtime. Plain glTF — the
    /// angle-picking shader lives in the `godot-mog` companion runtime.
