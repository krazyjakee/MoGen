...
pub(super) struct FileState {
    ...
    /// When `true`, the next Modify call also captures a thumbnail of the
    /// current scene and attaches it as a vision input alongside the
    /// edit prompt. Per-file so users can leave it on for one scene
    /// (where they're iterating against the render) and off for another
    /// (where the textual prompt is enough). Gated on the active
    /// provider supporting images at click time — the toggle stays
    /// enabled in the UI but the call falls back to text-only on
    /// non-vision providers so flipping providers doesn't silently
    /// strip the screenshot intent.
    pub(super) mod_include_screenshot: bool,
