...
use mogen_llm::{
    ...
    parse_seed_header, parse_style_header, parse_thinking_header, repair_message,
    stamp_style_header, validate_text, GenerateConfig, Provider, RepairConfig, Style,
    ThinkingLevel,
};
...
pub(crate) struct RepairArgs {
    ...
    /// CLI override; `None` falls through to the file's
    /// `meta(style=…)` attribute. Sticky across repair runs so a styled
    /// file regenerated by repair stays in style.
...
pub(crate) fn repair(args: RepairArgs) -> Result<()> {
    ...
    // Precedence: CLI flag > per-file header > none. The repair prompt
    // includes the broken DSL verbatim (so a stamped `meta(style=…)` round-
    // trips even without our intervention), but the model may regenerate
    // whole material/primitive blocks while fixing errors — applying the
    // style guidance up front keeps those rewrites in style.
