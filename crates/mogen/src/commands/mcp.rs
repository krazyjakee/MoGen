...
impl MogenMcp {
    ...
    #[tool(
        description = "Inspect .mog world bounds, dimensions, grounding and authored relationships, or read .glb structure (chunks, accessors, meshes, \
    ...
    #[tool(
        description = "Measure world triangle-surface distance between named .mog parts. Returns revision, metres, closest points, tolerance, evidence and budget limits. AABB overlap is not proof of contact."
    )]
    async fn measure(
        &self,
        Parameters(p): Parameters<MeasureArgs>,
    ) -> Result<CallToolResult, McpError> {
        let mut args = vec![
            "measure".into(),
            p.input.to_string_lossy().into_owned(),
        ...
        if let Some(t) = p.tolerance {
            args.extend(["--tolerance".into(), t.to_string()]);
        }
        if let Some(w) = p.max_work {
            args.extend(["--max-work".into(), w.to_string()]);
        }
        run_mogen(args).await
    }
    ...
    #[tool(
        description = "Generate/refine or resume the shared durable modeling session. Returns structured final/latest artifact paths, revisions, usage and stop status. Resume uses saved limits/context; inspect makes no model calls."
    )]
    async fn session(
        &self,
        Parameters(p): Parameters<SessionArgs>,
    ) -> Result<CallToolResult, McpError> {
        let mut args = vec![
            "session".into(),
            "--out-dir".into(),
            p.out_dir.to_string_lossy().into_owned(),
        ];
        push_opt(&mut args, "--prompt", p.prompt);
        push_path_opt(&mut args, "--brief", p.brief);
        push_path_opt(&mut args, "--input", p.input);
        push_flag(&mut args, "--resume", p.resume);
        push_flag(&mut args, "--inspect", p.inspect);
        push_opt(&mut args, "--provider", p.provider);
        push_opt(&mut args, "--model", p.model);
        push_opt(&mut args, "--seed", p.seed);
        push_opt(&mut args, "--temperature", p.temperature);
        push_opt(&mut args, "--thinking", p.thinking);
        push_flag(&mut args, "--draft", p.draft);
        push_flag(&mut args, "--glb", p.glb);
        push_flag(
            &mut args,
            "--experimental-guidance",
            p.experimental_guidance,
        );
        push_opt(&mut args, "--selected-part", p.selected_part);
        for path in p.reference {
            push_path_opt(&mut args, "--reference", Some(path));
        }
        for name in p.lock {
            push_opt(&mut args, "--lock", Some(name));
        }
        push_opt(&mut args, "--calls", p.calls);
        push_opt(&mut args, "--iterations", p.iterations);
        push_opt(&mut args, "--seconds", p.seconds);
        push_opt(&mut args, "--spend-usd", p.spend_usd);
        push_opt(&mut args, "--output-tokens", p.output_tokens);
        let mut result = run_mogen(args).await?;
        // The ordinary command's stdout is one JSON document, including stops.
